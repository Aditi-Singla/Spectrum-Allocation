...
void allocate::greedy_random_search(float inp_time, state start_state)
{
	...
	while(difftime(time(0), start) <= time_fact * inp_time*60 /*true*/ ) // TODO: Check for time elapsed
	{
		...
		else
		{
			...
			// int index = (int)(random_fraction()*(float)(current.list_of_bids.size()-1));

			// newstate.list_of_bids.erase(std::remove(newstate.list_of_bids.begin(), newstate.list_of_bids.end(), (current.list_of_bids[index])), newstate.list_of_bids.end());	

			// int random_index = (int)((random_fraction())*(float)(bid_company[bid_map[index].company].list_of_bids.size()-1));

			// std::cout<<"yo\n";
			// while(/*(current.list_of_bids[index] == bid_company[bid_map[index].company].list_of_bids[random_index]) || */!(check_collision(current.list_of_bids,bid_company[bid_map[index].company].list_of_bids[random_index])))
			// {
			// 	newstate.list_of_bids.push_back(current.list_of_bids[index]);
			// 	index = (int)(random_fraction()*(float)(current.list_of_bids.size()-1));

			// 	newstate.list_of_bids.erase(std::remove(newstate.list_of_bids.begin(), newstate.list_of_bids.end(), (current.list_of_bids[index])), newstate.list_of_bids.end());	
			// 	random_index = (int)((random_fraction())*(float)(bid_company[bid_map[index].company].list_of_bids.size()-1));		
			// 	std::cout<<random_index<<"yo\n";
			// }

			// newstate.list_of_bids.push_back(bid_company[bid_map[index].company].list_of_bids[random_index]);
